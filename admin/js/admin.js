...
(function ($, window, document, undefined) {
    ...
    function warning(status, msg, cmd) {
        window.alert(stripTag(msg || ip_location_block.msg[11].replace('%s', cmd) + ' (' + status + ')'));
    }

    function notice_html5() {
        warning(null, stripTag(ip_location_block.msg[9]));
    }

    function redirect(page, tab) {
        if (-1 !== window.location.href.indexOf(page)) {
            window.location = stripTag(page) + (tab ? '&' + stripTag(tab) : '') + '&' + (ip_location_block_auth.key ? (ip_location_block_auth.key + '=' + ip_location_block_auth.nonce) : '');
        }
    }
    ...
    function ajax_post(id, request, callback, objs) {
        ...
        request.action = ip_location_block.action;
        request.nonce = ip_location_block.nonce;

        var params = {
            type: 'POST',
            url: ip_location_block.url,
            data: request,
            dataType: 'json',
        };
...
//					chart.drawLine($(obj).attr('id'), 'datetime');
    ...
    function loadCookie(tabNo) {
        var i, cookie = (wpCookies !== undefined && wpCookies.getHash('ip-location-block')) || [];
    ...
    function saveCookie(cookie) {
        ...
        if (wpCookies !== undefined) {
            wpCookies.setHash(
                'ip-location-block', c, new Date(Date.now() + 2592000000),
                // `/wp-admin/`, `/wp-admin/network/` or `/wp/wp-admin/`
                window.location.pathname.replace(/\\/g, '/').match(/.*\//) || ip_location_block_auth.home + ip_location_block_auth.admin
            );
        }
    }
    ...
    function add_hidden_form(cmd) {
        $('body').append(
            ...
            '<form method="POST" id="' + ID('export-form') + '" action="' + ip_location_block.url.replace('ajax.php', 'post.php') + '">' +
            '<input type="hidden" name="action" value="' + ip_location_block.action + '" />' +
            '<input type="hidden" name="nonce" value="' + ip_location_block.nonce + '" />' +
    ...
    function initTable(tabNo, control, options, cookie) {
        ...
        $.extend(true, $.fn.dataTable.defaults, options, {
            ...
            // Language
            language: {
                emptyTable: ip_location_block.i18n[1],
                loadingRecords: ip_location_block.i18n[0],
                processing: ip_location_block.i18n[0],
                zeroRecords: ip_location_block.i18n[2],
                lengthMenu: '_MENU_',
                paginate: {
                    first: '&laquo;',
                    last: '&raquo;',
            ...
            drawCallback: function (settings) {
                ...
                if (n > settings.iDraw) {
                    elm.html(ip_location_block.i18n[0]);
                } else if (n === settings.iDraw) {
                    // 'No data available in table'
                    elm.html(ip_location_block.i18n[1]);
        ...
        var table = $(ID('#', control.tableID)).DataTable({
                ajax: {
                    ...
                    data: {
                        cmd: control.ajaxCMD,
                        action: ip_location_block.action,
                        nonce: ip_location_block.nonce
                    }
                },
        ...
        $(ID('#', 'bulk-action')).off('click').on('click', function (/*event*/) {
            ...
            if (!cmd) {
                ...
                warning(null, ip_location_block.msg[10]);
                return false;
            }
    ...
    $(function () {
        ...
        switch (tabNo) {
                ...
                ajax_post(null, {
                    ...
                    for (key in data) {
                        if (data.hasOwnProperty(key)) {
                            ...
                            if (!$this.find('#' + (id = ID('!', 'exception_admin_' + key))).size()) {
                                ...
                                if (1 & data[key]) {
                                    j.appendChild(add_icon(dfn, span, ip_location_block.msg[5], 'lock'));
                                }
                                if (2 & data[key]) {
                                    j.appendChild(add_icon(dfn, span, ip_location_block.msg[6], 'unlock'));
                                }
                    ...
                    $(ID('.', 'icon-find')).on('click', function (/*event*/) {
                        var $this = $(this),
                            ...
                            title = stripTag(ip_location_block.msg[8]),
                        ...
                        ajax_post('find-' + target, {
                            ...
                            for (val in data) {
                                if (data.hasOwnProperty(val)) {
                                    ...
                                    s = '<a class="ip-location-block-icon ip-location-block-icon-alert" href="' + ip_location_block_auth.sites[0] + ip_location_block_auth.admin + 'options-general.php' + // only main site
                                        '?page=ip-location-block&tab=4&s=' + encodeURIComponent(s) + '" title="' + title.replace('%s', s) + '" target="_blank"><span></span></a>';
                                    ...
                                    if (ext < 0) {
                                        ...
                                        $this.prepend(
                                            ...
                                            + ('admin' === target ? '/>' : 'name=ip_location_block_settings[exception][' + target + '][' + val + '] />')
                            ...
                            $(ID('#', 'find-' + target)).append(
                                ' ' + '<span class="ip-location-block-warn">' + stripTag(ip_location_block.msg[7].replace('%d', n)) + '</span>'
                            );
                        });
                ...
                $(ID('#', 'login-link')).on('click', function (/*event*/) {
                    ...
                    if ($this.hasClass(type)) {
                        ajax_post('login-loading', {
                            ...
                            $this.text(ip_location_block.msg[3]);
                            $this.removeClass(type).nextAll(ID('.', 'desc')).remove();
                            $('<p class="ip-location-block-desc"></p>')
                                .appendTo($this.parent())
                                .append(
                                    ip_location_block.msg[4],
                                    '<a href="' + data.link + '" title="' + ip_location_block.msg[1] + '" target=_blank>' + data.link + '</a></p>'
                                );
                        });
                    } else {
                        confirm(ip_location_block.msg[0], function () {
                            ajax_post('login-loading', {
                                ...
                                $this.text(ip_location_block.msg[2]);
            ...
            case 1:
                ...
                initTable(tabNo, {
                    ...
                    columns: [
                        ...
                        {title: ip_location_block.i18n[3]}, // 1 IP address
                        {title: ip_location_block.i18n[4]}, // 2 Country code
                        {title: ip_location_block.i18n[5]}, // 3 AS number
                        {title: ip_location_block.i18n[6]}, // 4 Host name
                        {title: ip_location_block.i18n[7]}, // 5 Target
                        {title: ip_location_block.i18n[8]}, // 6 Login fail/Call
                        {title: ip_location_block.i18n[9]}  // 7 Elapsed[sec]
                    ],
                    columnDefs: [
                ...
                var control = {
                    ...
                    live_start = function (cmd) {
                        ...
                        ajax_post(cmd === undefined ? 'live-loading' : null, {
                            ...
                            if (cmd === undefined) {
                                // keep updating
                                timer_start = window.setTimeout(live_start, ip_location_block.interval * 1000);
                            }
                        });
                    },
                    ...
                    live_pause = function () {
                        live_stop('live-pause', function () {
                            ...
                            timer_pause = window.setTimeout(function () {
                                ...
                                live_stop();
                            }, ip_location_block.timeout * 1000);
                        });
                    },
            ...
            case 2:
                ...
                var LocationMap = L.map(ID('#', 'map').replace('#', '')).setView([0, 0], 1);
                ...
                $(ID('@', 'get_location')).on('click', function (/*event*/) {
                    ...
                    if (ip && list) {
                        ...
                        LocationMap.setView([0, 0], 1);
                        for (var i in LocationMapMarkers) {
                            LocationMap.removeLayer(LocationMapMarkers[i])
                        }
                        LocationMapMarkers = [];
                        ...
                        ajax_post('loading', {
                            ...
                            apis.html(
                                ...
                                '<div id="ip-location-block-geoinfo"></div>'
                            ).fadeIn('slow')
                                ...
                                .on('click', 'a.nav-tab', function () {
                                    ...
                                    if (json.latitude && json.longitude) {
                                        var marker = L.marker([json.latitude, json.longitude]).addTo(LocationMap);
                                        LocationMap.setView([json.latitude, json.longitude], 11);
                                        marker.bindPopup('<h4>' + ip + '</h4><ul>' + info + '</ul>');
